...
String lat = "";
String lon = "";
String weKey = "c98d7cb82cdc49b7a20f8367e865868a";
...
void scan_wifi_task(void *pvParameters)
{
    ...
    lv_label_set_text(ui_Label12, "Please connect to WiFi");
    vTaskDelete(NULL);
}
...
void connect_wifi_task(void *pvParameters)
{
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
        ...
        xTaskCreatePinnedToCore(getWeather, "getWeather", 8192, NULL, 1, NULL, 1);
    }
    else
    {
...
void getNtpTime(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        topDateText = yearText + "/" + mouthText + "/" + dayText;
        // Serial.printf("currentTime_week = %d",currentTime_week);
        // Serial.println();
        if (currentTime_week == 7)
        {
            weekText = "Saturday";
        }
...
float *getCityPosition(String ips)
{
    ...
    // 设置请求头中的User-Agent
    httpClient.setUserAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.0.0 Safari/537.36 Edg/134.0.0.0");
...
void getWeather(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        wifiClient.setInsecure();
        String URL = "https://devapi.qweather.com/v7/weather/now?location=" + city_pos + "&key=c98d7cb82cdc49b7a20f8367e865868a&lang=en";
